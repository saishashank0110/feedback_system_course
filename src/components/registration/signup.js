...
class SignUp extends Component {   
    ...
    signup(Event){
        ...
        if(params.role==='student'&& (params.id[4]!=='-' || params.id[7]!=='-' || params.id[11]!=='-' || params.id.length!==15)){
            format=false
            Swal.fire({
                title: 'error',
                text: 'Student ID format: xxxx-xx-xxx-xxx',
                icon: 'error',
                confirmButtonText: 'retry'
              }).then((result) =>{
                if (result.isConfirmed) {    
        ...
        if(params.role==='admin'&& (params.id[0]!=='A' || params.id.length!==5)){
            format=false
            Swal.fire({
                title: 'error',
                text: 'Admin ID should be of format: Axxxx',
                icon: 'error',
                confirmButtonText: 'retry'
              }).then((result) =>{
                if (result.isConfirmed) {    
